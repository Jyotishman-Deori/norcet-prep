...
function RoundRunner({ T, run, flashpoint, soundOn, setSoundOn, onExit, onFinishRun }) {
  ...
  // The guard lives HERE, not in the parent: the rounds already titrated are
  // held in this component's ref and are not handed up until the run ends, so
  // the parent genuinely does not know what is on the table yet. Backing out
  // mid-run used to go straight home and bin all of it.
  // `progress: idx + 1` so the CURRENT round counts: a titration you are three
  // minutes into is worth a confirm even before it has scored a single coin.
  ...
  return (
    <>
      ...
      <RoundView key={drug.id} T={T} drug={drug} roundNo={idx + 1} total={run.length}
                 ...
                 onExit={requestExit} onNext={onRoundDone} />
    </>
  );
}
...
function DoneView({ T, results, flashpoint, onPlayAgain, onComplete, onBack }) {
  ...
  return (
    <div className="anim-fadeup" style={{ minHeight: '100vh', background: T.bg }}>
      <TopBar title="Drip Zone" onBack={finish} />
